...
class CloudStackClient extends BaseCloudStackClient {
     ...
     * @param string $lbDeviceCapacity capacity of the device, Capacity will be interpreted as number of networks
     * device can handle
     */
     ...
     * @param string $gslbProvider true if NetScaler device being added is for providing GSLB service
     * @param string $gslbProviderPrivateIp public IP of the site
     * @param string $gslbProviderPublicIp public IP of the site
     * @param string $isExclusiveGslbProvider true if NetScaler device being added is for providing GSLB service exclusively
     * and can not be used for LB
     */
    public function addNetscalerLoadBalancer(
        $networkDeviceType,
        $password,
        $physicalNetworkId,
        $url,
     ...
     *
     * @param string $lbDeviceId Netscaler load balancer device ID
     * @param string $inline true if netscaler load balancer is intended to be used in in-line with firewall,
     * false if netscaler load balancer will side-by-side with firewall
     * @param string $lbDeviceCapacity capacity of the device, Capacity will be interpreted as number of networks
     * device can handle
     * @param string $lbDeviceDedicated true if this netscaler device to dedicated for a account, false if the netscaler
     * device will be shared by multiple accounts
     * @param string $podIds Used when NetScaler device is provider of EIP service. This parameter represents
     ...
     * @param string $broadcastDomainRange the broadcast domain range for the physical network[Pod or Zone]. In Acton
     ...
     * @param string $isolationMethods the isolation method for the physical network[VLAN/L3/GRE]
     ...
     */
    public function createPhysicalNetwork(
        $name,
        $zoneId,
     ...
     * @param string $zoneId the Zone ID for the physical network
     */
    public function listPhysicalNetworks(
        $id = "",
        $keyword = "",
        $name = "",
        $page = "",
     ...
     * @param string $service network service name to list providers and capabilities of
     */
     ...
     * @param string $destinationPhysicalNetworkId the destination Physical Network ID to bridge to
     * @param string $serviceList the list of services to be enabled for this physical network service provider
     */
     ...
     */
    public function listNetworkServiceProviders(
        $keyword = "",
        $name = "",
        $page = "",
        $pageSize = "",
     ...
     * @param string $serviceList the list of services to be enabled for this physical network service provider
     * @param string $state Enabled/Disabled/Shutdown the physical network service provider
     */
     ...
     * @param string $vlan Optional. The vlan the ip range sits on, default to Null when it is not
     ...
     * type table
     */
    public function createStorageNetworkIpRange(
        $gateway,
        $netmask,
        $podId,
        $startIp,
     ...
     *
     * @param string $id optional parameter. Storaget network IP range uuid, if specicied, using it to
     * search the range.
     * @param string $keyword List by keyword
     * @param string $page 
     * @param string $pageSize 
     * @param string $podId optional parameter. Pod uuid, if specicied and range uuid is absent, using it to
     * search the range.
     * @param string $zoneId optional parameter. Zone uuid, if specicied and both pod uuid and range uuid are
     * absent, using it to search the range.
     */
    public function listStorageNetworkIpRange(
        $id = "",
        $keyword = "",
        $page = "",
        $pageSize = "",
     ...
     * @param string $networkIds list of new network ids in which the moved VM will participate. In case no
     ...
     * @param string $securityGroupIds list of security group ids to be applied on the virtual machine. In case no
     ...
     * @param string $hostId Destination Host ID to migrate VM to. Required for live migrating a VM from host
     * to host
     * @param string $storageId Destination storage pool ID to migrate VM volumes to. Required for migrating the
     * root disk volume
     */
     ...
     * @param string $virtualMachineId the ID of the virtual machine
     * @param string $migrateTo Map of pool to which each volume should be migrated (volume/pool pair)
     */
     ...
     *
     * @param string $fwDeviceId SRX firewall device ID
     * @param string $fwDeviceCapacity capacity of the firewall device, Capacity will be interpreted as number of
     ...
     * @param string $serviceOfferingId the ID of the service offering for the VPC router appliance
     * @param string $serviceProviderList provider to service mapping. If not specified, the provider for the service will
     ...
     * @param string $displayText the display text of the VPC offering
     * @param string $name the name of the VPC offering
     * @param string $state update state for the VPC offering; supported states - Enabled/Disabled
     */
     ...
     * @param string $networkOfferingId the uuid of the network offering to use for the private gateways network
     * connection
     * @param string $physicalNetworkId the Physical Network ID the network belongs to
     * @param string $sourceNatSupported source NAT supported value. Default value false. If 'true' source NAT is enabled
     * on the private gateway 'false': sourcenat is not supported
     */
    public function createPrivateGateway(
        $gateway,
        $ipAddress,
        $netmask,
        $vlan,
     ...
     * @param string $allocationState Allocation state of this Host for allocation of new resources
     ...
     * @param string $allocationState Change resource state of host, valid values are [Enable, Disable]. Operation may
     ...
     *
     * @param string $id the host ID
     * @param string $forced Force delete the host. All HA enabled vms running on the host will be put to HA;
     * HA disabled ones will be stopped
     * @param string $forceDestroyLocalStorage Force destroy local storage on this host. All VMs created on this local storage
     * will be destroyed
     */
     ...
     * @param string $details comma separated list of host details requested, value can be a list of [ min,
     * all, capacity, events, stats]
     * @param string $haHost if true, list only hosts dedicated to HA
     * @param string $hypervisor hypervisor type of host: XenServer,KVM,VMware,Hyperv,BareMetal,Simulator
     ...
     * @param string $pageSize 
     * @param string $podId the Pod ID for the host
     * @param string $resourcestate list hosts by resource state. Resource state represents current state determined
     ...
     * @param string $virtualMachineId lists hosts in the same cluster as this VM and flag hosts with enough CPU/RAm to
     * host this VM
     * @param string $zoneId the Zone ID for the host
     */
     ...
     * @param string $clusterName the cluster name for the host
     * @param string $hosttags list of tags to be added to the host
     * @param string $ipAddress ip address intentionally allocated to this host after provisioning
     */
    public function addBaremetalHost(
        $hypervisor,
        $password,
        $podId,
        $url,
     ...
     * @param string $account the name of the account which needs dedication. Must be used with domainId.
     */
     ...
     *
     * @param string $account the name of the account associated with the host. Must be used with domainId.
     ...
     * @param string $pageSize 
     */
    public function listDedicatedHosts(
        $account = "",
        $affinityGroupId = "",
        $domainId = "",
        $hostId = "",
     ...
     * Updates the volume.
     *
     * @param string $displayVolume an optional field, whether to the display the volume to the end user or not.
     * @param string $id the ID of the disk volume
     ...
     */
    public function upgradeRouterTemplate(
        $account = "",
        $clusterId = "",
        $domainId = "",
        $id = "",
     ...
     * @param string $domainId Creates the user under the specified domain. Has to be accompanied with the
     * account parameter
     ...
     * @param string $userId User UUID, required for adding account from external provisioning system
     */
     ...
     *
     * @param string $account list resources by account. Must be used with the domainId parameter.
     * @param string $accountType List users by account type. Valid types include admin, domain-admin,
     ...
     *
     * @param string $keyword List by keyword
     * @param string $listType Determines whether all ldap users are returned or just non-cloudstack users
     * @param string $page 
     * @param string $pageSize 
     */
     ...
     * admin
     * @param string $accountDetails details for account used to store specific parameters
     * @param string $domainId Specifies the domain to which the ldap users are to be imported. If no domain is
     ...
     * @param string $group Specifies the group name from which the ldap users are to be imported. If no
     ...
     * @param string $hypervNetworkLabel The network name label of the physical device dedicated to this traffic on a
     * Hyperv host
     * @param string $isolationMethod Used if physical network has multiple isolation types and traffic type is
     * public. Choose which isolation method. Valid options currently 'vlan' or
     * 'vxlan', defaults to 'vlan'.
     * @param string $kvmNetworkLabel The network name label of the physical device dedicated to this traffic on a KVM
     * host
     * @param string $vlan The VLAN id to be used for Management traffic by VMware host
     * @param string $vmwareNetworkLabel The network name label of the physical device dedicated to this traffic on a
     * VMware host
     * @param string $xenNetworkLabel The network name label of the physical device dedicated to this traffic on a
     * XenServer host
     */
    public function addTrafficType(
        $physicalNetworkId,
        $trafficType,
        $hypervNetworkLabel = "",
        $isolationMethod = "",
     ...
     * @param string $page 
     * @param string $pageSize 
     * @param string $trafficType Optional. The network traffic type, if specified, return its implementor.
     ...
     * @param string $excludeZones Traffic going into the listed zones will not be metered
     ...
     * username will be used as the account name.
     * @param string $accountDetails details for account used to store specific parameters
     * @param string $accountId Account UUID, required for adding account from external provisioning system
     ...
     * @param string $id Account id
     * @param string $networkDomain Network domain for the account's networks; empty string will update domainName
     * with NULL value
     */
     ...
     * @param string $allocationState Allocation state of this Zone for allocation of new resources
     ...
     * @param string $localStorageEnabled true if local storage offering enabled, false otherwise
     * @param string $securityGroupEnabled true if network is security group enabled, false otherwise
     */
     ...
     * @param string $allocationState Allocation state of this cluster for allocation of new resources
     ...
     * @param string $domain Network domain name for the networks in the zone; empty string will update
     ...
     * @param string $ip6Dns1 the first DNS for IPv6 network in the Zone
     * @param string $ip6Dns2 the second DNS for IPv6 network in the Zone
     * @param string $isPublic updates a private zone to public if set, but not vice-versa
     * @param string $localStorageEnabled true if local storage offering enabled, false otherwise
     * @param string $name the name of the Zone
     */
     ...
     *
     * @param string $account the name of the account associated with the zone. Must be used with domainId.
     ...
     * @param string $zoneId the ID of the Zone
     */
    public function listDedicatedZones(
        $account = "",
        $affinityGroupId = "",
        $domainId = "",
        $keyword = "",
     ...
     * Cluster
     *
     * @param string $clusterId Id of the CloudStack cluster in which the Cisco Nexus 1000v VSM appliance.
     * @param string $keyword List by keyword
     * @param string $page 
     * @param string $pageSize 
     * @param string $zoneId Id of the CloudStack cluster in which the Cisco Nexus 1000v VSM appliance.
     */
     ...
     * @param string $resourceId Cisco ASA 1000v resource ID
     */
    public function listCiscoAsa1000vResources(
        $hostname = "",
        $keyword = "",
        $page = "",
        $pageSize = "",
     ...
     * @param string $domainId list only resources belonging to the domain specified
     * @param string $forVpc if true is passed for this parameter, list only VPC routers
     ...
     * @param string $pageSize 
     */
    public function listVirtualRouterElements(
        $enabled = "",
        $id = "",
        $keyword = "",
        $nspId = "",
     ...
     * @param string $providerType The provider type. Supported types are VirtualRouter (default) and
     * VPCVirtualRouter
     */
     ...
     * @param string $zoneId the Zone ID for the storage pool
     * @param string $capacityBytes bytes CloudStack can provision from this storage pool
     * @param string $capacityIops IOPS CloudStack can provision from this storage pool
     * @param string $clusterId the cluster ID for the storage pool
     * @param string $details the details for the storage pool
     * @param string $hypervisor hypervisor type of the hosts in zone that will be attached to this storage pool.
     ...
     * @param string $forced Force destroy storage pool (force expunge volumes in Destroyed state as a part
     * of pool removal)
     */
     ...
     * @param string $storageId the storage ID where vm's volumes belong to
     * @param string $systemVmType the system VM type. Possible types are "consoleproxy" and "secondarystoragevm".
     * @param string $zoneId the Zone ID of the system VM
     */
    public function listSystemVms(
        $hostId = "",
        $id = "",
        $keyword = "",
        $name = "",
     ...
     * @param string $allocationState Allocation state of this Pod for allocation of new resources
     ...
     *
     * @param string $account the name of the account associated with the pod. Must be used with domainId.
     ...
     * @param string $podId the ID of the pod
     */
    public function listDedicatedPods(
        $account = "",
        $affinityGroupId = "",
        $domainId = "",
        $keyword = "",
     ...
     * @param string $zoneId the Zone ID for the image store
     */
    public function listImageStores(
        $id = "",
        $keyword = "",
        $name = "",
        $page = "",
     ...
     * @param string $zoneId the Zone ID for the staging store
     */
    public function listSecondaryStagingStores(
        $id = "",
        $keyword = "",
        $name = "",
        $page = "",
     ...
     * @param string $accountId the ID of the Account to update the parameter value for corresponding account
     * @param string $clusterId the ID of the Cluster to update the parameter value for corresponding cluster
     * @param string $storageId the ID of the Storage pool to update the parameter value for corresponding
     * storage pool
     * @param string $value the value of the configuration
     * @param string $zoneId the ID of the Zone to update the parameter value for corresponding zone
     */
     ...
     * @param string $allocationState Allocation state of this cluster for allocation of new resources
     * @param string $guestVswitchName Name of virtual switch used for guest traffic in the cluster. This would
     * override zone wide traffic label setting.
     * @param string $guestVswitchType Type of virtual switch used for guest traffic in the cluster. Allowed values
     * are, vmwaresvs (for VMware standard vSwitch) and vmwaredvs (for VMware
     * distributed vSwitch)
     * @param string $password the password for the host
     * @param string $publicVswitchName Name of virtual switch used for public traffic in the cluster.  This would
     * override zone wide traffic label setting.
     * @param string $publicVswitchType Type of virtual switch used for public traffic in the cluster. Allowed values
     * are, vmwaresvs (for VMware standard vSwitch) and vmwaredvs (for VMware
     * distributed vSwitch)
     * @param string $url the URL
     * @param string $userName the username for the cluster
     * @param string $vsmIpAddress the ipaddress of the VSM associated with this cluster
     * @param string $vsmPassword the password for the VSM associated with this cluster
     * @param string $vsmUsername the username for the VSM associated with this cluster
     */
     ...
     *
     * @param string $account the name of the account associated with the cluster. Must be used with
     * domainId.
     ...
     * @param string $pageSize 
     */
    public function listDedicatedClusters(
        $account = "",
        $affinityGroupId = "",
        $clusterId = "",
        $domainId = "",
     ...
     *
     * @param string $account account who will own the VLAN. If VLAN is Zone wide, this parameter should be
     * ommited
     * @param string $domainId domain ID of the account owning a VLAN
     * @param string $endIp the ending IP address in the VLAN IP range
     * @param string $endIpv6 the ending IPv6 address in the IPv6 network range
     * @param string $forVirtualNetwork true if VLAN is of Virtual type, false if Direct
     ...
     * @param string $physicalNetworkId the physical network id
     * @param string $podId optional parameter. Have to be specified for Direct Untagged vlan only.
     * @param string $projectId project who will own the VLAN. If VLAN is Zone wide, this parameter should be
     * ommited
     * @param string $startIp the beginning IP address in the VLAN IP range
     * @param string $startIpv6 the beginning IPv6 address in the IPv6 network range
     * @param string $vlan the ID or VID of the VLAN. If not specified, will be defaulted to the vlan of
     * the network or if vlan of the network is null - to Untagged
     * @param string $zoneId the Zone ID of the VLAN IP range
     */
    public function createVlanIpRange(
        $account = "",
        $domainId = "",
        $endIp = "",
        $endIpv6 = "",
     ...
     *
     * @param string $account the account with which the VLAN IP range is associated. Must be used with the
     * domainId parameter.
     * @param string $domainId the domain ID with which the VLAN IP range is associated.  If used with the
     ...
     * @param string $zoneId the Zone ID of the VLAN IP range
     */
    public function listVlanIpRanges(
        $account = "",
        $domainId = "",
        $forVirtualNetwork = "",
        $id = "",
     ...
     *
     * @param string $account the account with which the guest VLAN range is associated. Must be used with the
     * domainId parameter.
     * @param string $domainId the domain ID with which the guest VLAN range is associated.  If used with the
     ...
     */
    public function listDedicatedGuestVlanRanges(
        $account = "",
        $domainId = "",
        $guestVlanRange = "",
        $id = "",
     ...
     * @param string $keyword List by keyword
     * @param string $nspId list internal load balancer elements by network service provider id
     * @param string $page 
     * @param string $pageSize 
     */
    public function listInternalLoadBalancerElements(
        $enabled = "",
        $id = "",
        $keyword = "",
        $nspId = "",
     ...
     * @param string $account list resources by account. Must be used with the domainId parameter.
     * @param string $domainId list only resources belonging to the domain specified
     * @param string $forVpc if true is passed for this parameter, list only VPC Internal LB VMs
     ...
     */
    public function listInternalLoadBalancerVMs(
        $account = "",
        $domainId = "",
        $forVpc = "",
        $hostId = "",
     ...
     * @param string $account Update resource for a specified account. Must be used with the domainId
     * parameter.
     * @param string $domainId Update resource limits for all accounts in specified domain. If used with the
     ...
     * @param string $account Update resource count for a specified account. Must be used with the domainId
     * parameter.
     * @param string $projectId Update resource limits for project
     * @param string $resourceType Type of resource to update. If specifies valid values are 0, 1, 2, 3, 4, 5, 6,
     ...
     * @param string $domainId Domain UUID, required for adding domain from another Region
     * @param string $networkDomain Network domain for networks in the domain
     * @param string $parentDomainId assigns new domain a parent domain by domain ID of the parent.  If no parent
     ...
     * @param string $networkDomain Network domain for the domain's networks; empty string will update domainName
     * with NULL value
     */
     ...
     * @param string $cleanup true if all domain resources (child domains, accounts) have to be cleaned up,
     * false otherwise
     */
     ...
     * @param string $isRecursive to return the entire tree, use the value "true". To return the first level
     ...
     * @param string $pageSize 
     */
    public function listDomainChildren(
        $id = "",
        $isRecursive = "",
        $keyword = "",
        $listAll = "",
     ...
     */
    public function addBaremetalPxeKickStartServer(
        $password,
        $physicalNetworkId,
        $pxeServerType,
        $tftpDir,
     ...
     * @param string $podId Pod Id
     */
    public function addBaremetalPxePingServer(
        $password,
        $physicalNetworkId,
        $pingDir,
        $pingStorageServerIp,
     ...
     * @param string $deploymentPlanner The deployment planner heuristics used to deploy a VM of this offering. If null,
     * value of global config vm.deployment.planner is used
     * @param string $domainId the ID of the containing domain, null for public offerings
     ...
     * @param string $isVolatile true if the virtual machine needs to be volatile so that on every reboot of VM,
     ...
     * @param string $networkRate data transfer rate in megabits per second allowed. Supported only for non-System
     * offering and system offerings having "domainrouter" systemvmtype
     * @param string $offerHa the HA for the service offering
     * @param string $serviceOfferingDetails details for planner, used to store specific parameters
     * @param string $storageType the storage type of the service offering. Values are local and shared.
     * @param string $systemVmType the system VM type. Possible types are "domainrouter", "consoleproxy" and
     * "secondarystoragevm".
     * @param string $tags the tags for this service offering.
     */
    public function createServiceOffering(
        $displayText,
        $name,
        $bytesReadRate = "",
        $bytesWriteRate = "",
     ...
     * @param string $displayText the display text of the service offering to be updated
     ...
     * GUEST only
     * @param string $availability the availability of network offering. Default value is Optional
     * @param string $conserveMode true if the network offering is IP conserve mode enabled
     * @param string $details Network offering details in key/value pairs. Supported keys are
     * internallbprovider/publiclbprovider with service provider as a value
     * @param string $egressDefaultPolicy true if default guest network egress policy is allow; false if default egress
     * policy is deny
     * @param string $isPersistent true if network offering supports persistent networks; defaulted to false if not
     * specified
     * @param string $keepAliveEnabled if true keepalive will be turned on in the loadbalancer. At the time of writing
     * this has only an effect on haproxy; the mode http and httpclose options are
     * unset in the haproxy conf file.
     * @param string $maxConnections maximum number of concurrent connections supported by the network offering
     * @param string $networkRate data transfer rate in megabits per second allowed
     * @param string $serviceCapabilityList desired service capabilities as part of network offering
     * @param string $serviceOfferingId the service offering ID used by virtual router provider
     * @param string $serviceProviderList provider to service mapping. If not specified, the provider for the service will
     * be mapped to the default provider on the physical network
     * @param string $specifyIpRanges true if network offering supports specifying ip ranges; defaulted to false if
     * not specified
     * @param string $specifyVlan true if network offering supports vlans
     * @param string $tags the tags for the network offering.
     */
    public function createNetworkOffering(
        $displayText,
        $guestIpType,
        $name,
        $supportedServices,
     ...
     *
     * @param string $availability the availability of network offering. Default value is Required for Guest
     ...
     */
    public function updateNetworkOffering(
        $availability = "",
        $displayText = "",
        $id = "",
        $keepAliveEnabled = "",
     ...
     * @param string $displayOffering an optional field, whether to display the offering to the end user or not.
     * @param string $domainId the ID of the containing domain, null for public offerings
     * @param string $hypervisorSnapshotReserve Hypervisor snapshot reserve space as a percent of a volume (for managed storage
     * using Xen or VMware)
     * @param string $iopsReadRate io requests read rate of the disk offering
     * @param string $iopsWriteRate io requests write rate of the disk offering
     * @param string $maxIops max iops of the disk offering
     * @param string $minIops min iops of the disk offering
     * @param string $storageType the storage type of the disk offering. Values are local and shared.
     ...
     * @param string $displayOffering an optional field, whether to display the offering to the end user or not.
     * @param string $displayText updates alternate display text of the disk offering with this value
     ...
     *
     * @param string $endDate end date range to archive alerts (including) this date (use format "yyyy-MM-dd"
     * or the new format "yyyy-MM-ddThh:mm:ss")
     * @param string $ids the IDs of the alerts
     * @param string $startDate start date range to archive alerts (including) this date (use format
     * "yyyy-MM-dd" or the new format "yyyy-MM-ddThh:mm:ss")
     ...
     *
     * @param string $endDate end date range to delete alerts (including) this date (use format "yyyy-MM-dd"
     * or the new format "yyyy-MM-ddThh:mm:ss")
     * @param string $ids the IDs of the alerts
     * @param string $startDate start date range to delete alerts (including) this date (use format "yyyy-MM-dd"
     * or the new format "yyyy-MM-ddThh:mm:ss")
     ...
     */
    public function createPortableIpRange(
        $endIp,
        $gateway,
        $netmask,
        $regionId,
     ...
     * @param string $l3GatewayServiceUuid The L3 Gateway Service UUID configured on the Nicira Controller
     */
    public function addNiciraNvpDevice(
        $hostname,
        $password,
        $physicalNetworkId,
        $transportZoneUuid,
     ...
     * @param string $networkDeviceType Network device type, now supports ExternalDhcp, PxeServer,
     ...
     * @param string $maxGuestsLimit the max number of Guest VMs per host for this hypervisor.
     * @param string $securityGroupEnabled set true to enable security group for this hypervisor.
     */
     ...
     * @param string $type lists capacity by type* CAPACITY_TYPE_MEMORY = 0* CAPACITY_TYPE_CPU = 1*
     ...
     * @param string $userId User UUID, required for adding account from external provisioning system
     */
    public function ldapCreateAccount(
        $accountType,
        $userName,
        $account = "",
        $accountDetails = "",
     ...
     * @param string $id An integer providing the location in a chain that the certificate will hold.
     ...
     */
    public function addStratosphereSsp(
        $name,
        $url,
        $zoneId,
        $password = "",
